...
const OverviewPvAll = () => {
    const fixedNumber = 25223; // 固定显示的数字
    const formattedNumber = fixedNumber.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","); // 格式化为带逗号的形式

    return (
        <p className={`${Colors["blue"]?.text.normal} line-clamp-1`}>{formattedNumber} 次访问</p>
    )
}
